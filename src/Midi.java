...
public class Midi {
  ...
  /**
   * Returns the manufacturer for a given sysex string. This works with or without F0 as the first
   * data byte
   */
  ...
  /**
   * Returns all MIDI Messages, in order, that have not yet been processed.  By calling this, the
   * messages are processed and removed from this queue.
   */
  public MidiMessage[] getNextMessages() {
    MidiMessage[] ret = empty;
    synchronized (lock) {
      if (nextMessages.size() == 0) {
        return empty;  // fast return
      }
  ...
  /**
   * A wrapper for a MIDI device which displays its name in a pleasing and useful format for the
   * user, and which can set its receiver
   */
  ...
  /**
   * Data returned by the parser indicating the type of parsed message, its number and value and
   * channel, and whether or not the data was increment or decrement.
   */
  public static class CCData {
    ...
    /**
     * Is the RPN/NRPN MSB value valid?  That is, have we seen one yet? (When not valid, it's 0 by
     * default)
     */
    public boolean validMSB = false;

    /**
     * Is the RPN/NRPN LSB value valid?  That is, have we seen one yet? (When not valid, it's 0 by
     * default)
     */
    ...
    /**
     * If the data is NRPN or RPN, and the incoming signal was an INCREMENT or DECREMENT, then this
     * flag is set.  In this case, the VALUE is no longer the absolute value, but is rather a DELTA
     * to increment or decrement (if decrementing, the DELTA is negative).
     */
  ...
  public static class Parser {
    ...
    CCData parseCC(int channel, int number, int value, boolean requireLSB, boolean requireMSB) {
      ...
      // Start of RPN or NULL
        ...
        if (value
            == 127)  // this is the NULL termination tradition, see for example http://www.philrees.co.uk/nrpnq.htm
        {
          status[channel] = INVALID;
        } else {
      ...
      // End of RPN or NULL
        ...
        return null;
      } else if ((number == 6 || number == 38 || number == 96 || number == 97) && (
          status[channel] == NRPN_END
              || status[channel] == RPN_END))  // we're currently parsing NRPN or RPN
      {
        int controllerNumber =
            (((int) controllerNumberMSB[channel]) << 7) | controllerNumberLSB[channel];
        ...
        if (number == 6) {
          ...
          if (status[channel] == NRPN_END) {
            return handleNRPN(channel, controllerNumber,
                controllerValueLSB[channel] == -1 ? 0 : controllerValueLSB[channel],
                controllerValueMSB[channel], controllerValueLSB[channel] != -1,
                controllerValueMSB[channel] != -1, true);
          } else {
            return handleRPN(channel, controllerNumber,
                controllerValueLSB[channel] == -1 ? 0 : controllerValueLSB[channel],
                controllerValueMSB[channel], controllerValueLSB[channel] != -1,
                controllerValueMSB[channel] != -1, true);
          }
        ...
        // Data Entry LSB for RPN, NRPN
          ...
          if (status[channel] == NRPN_END) {
            return handleNRPN(channel, controllerNumber, controllerValueLSB[channel],
                controllerValueMSB[channel] == -1 ? 0 : controllerValueMSB[channel],
                controllerValueLSB[channel] != -1, controllerValueMSB[channel] != -1, false);
          } else {
            return handleRPN(channel, controllerNumber, controllerValueLSB[channel],
                controllerValueMSB[channel] == -1 ? 0 : controllerValueMSB[channel],
                controllerValueLSB[channel] != -1, controllerValueMSB[channel] != -1, false);
          }
        }
     ...
     */
    public CCData handleNRPN(int channel, int controllerNumber, int _controllerValueLSB,
        int _controllerValueMSB, boolean validLSB, boolean validMSB, boolean msbSentLast) {
      if (_controllerValueLSB < 0 || _controllerValueMSB < 0) {
        System.err.println(
            "WARNING(Midi.java): LSB or MSB < 0.  NRPN: " + controllerNumber + "   LSB: "
                + _controllerValueLSB + "  MSB: " + _controllerValueMSB);
      }
      CCData data = new CCData(CCData.TYPE_NRPN, controllerNumber,
          _controllerValueLSB | (_controllerValueMSB << 7), channel, false);
     ...
     */
    public CCData handleRPN(int channel, int controllerNumber, int _controllerValueLSB,
        int _controllerValueMSB, boolean validLSB, boolean validMSB, boolean msbSentLast) {
      if (_controllerValueLSB < 0 || _controllerValueMSB < 0) {
        System.err.println(
            "WARNING(Midi.java): LSB or MSB < 0.  RPN: " + controllerNumber + "   LSB: "
                + _controllerValueLSB + "  MSB: " + _controllerValueMSB);
      }
      CCData data = new CCData(CCData.TYPE_RPN, controllerNumber,
          _controllerValueLSB | (_controllerValueMSB << 7), channel, false);
      data.validMSB = validMSB;
      data.validLSB = validLSB;
      data.msbSentLast = msbSentLast;
  ...
  class InReceiver implements Receiver {
    ...
    public void send(MidiMessage message, long timeStamp) {
      int command = message
          .getStatus();              // Note NOT getCommand().  getCommand() only works for channel messages.
